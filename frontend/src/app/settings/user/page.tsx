...
import {
  User, Building2, Copy, Check, Lock, Loader2, Pencil, X, Save, Info,
} from "lucide-react"
...
function ReadonlyField({ label, value, mono = false, copy }: { label: string; value: string; mono?: boolean, copy:boolean}) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b last:border-b-0 gap-3">
      ...
      <span className={`text-sm flex-1 min-w-0 truncate ${mono ? "font-mono text-xs text-muted-foreground" : "font-medium"}`}>
        {value}
      </span>
...
function UserProfileCard() {
  ...
  if (!user) return <p className="text-sm text-muted-foreground">Нет данных о пользователе</p>
    ...
    <Card>
        ...
        <div className="flex items-center justify-between">
          ...
          {!editing ? (
            ...
            <div className="flex gap-1.5">
              <Button variant="ghost" size="sm" className="h-8 gap-1" onClick={handleCancel} disabled={isPending}>
                <X className="h-3.5 w-3.5" /> Отмена
              </Button>
              <Button size="sm" className="h-8 gap-1.5" onClick={() => mutate()} disabled={isPending}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="rounded-lg border bg-card p-3 space-y-3">
          <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">Редактируемые поля</p>
          <div className="space-y-2">
            <div className="space-y-1">
              <Label htmlFor="u-name" className="text-sm">Имя</Label>
              {editing ? (
                <Input id="u-name" value={name} onChange={(e) => setName(e.target.value)} className="h-8 text-sm" placeholder="Введите имя" />
              ) : (
                <div className="flex items-center gap-2 h-8 px-3 rounded-md border bg-muted/30 text-sm">
                  <span className="flex-1">{user.name || <span className="text-muted-foreground italic">не указано</span>}</span>
                </div>
              )}
            </div>
            <div className="space-y-1">
              ...
              {editing ? (
                <Input id="u-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-8 text-sm" placeholder="email@example.com" />
              ) : (
                <div className="flex items-center gap-2 h-8 px-3 rounded-md border bg-muted/30 text-sm">
        ...
        <div className="rounded-lg border bg-muted/20 p-3">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">Только чтение</p>
          </div>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-xs">Текущий тенант</CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        {isLoading ? (
          ...
          <>
            <div className="rounded-lg border bg-card p-3">
              <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide mb-1">Идентификаторы</p>
            ...
            <div className="flex items-start gap-2 rounded-lg border border-dashed p-3 text-xs text-muted-foreground">
              ...
              <p>Редактирование данных компании недоступно через API. Обратитесь к администратору.</p>
            </div>
          </>
        )}
      </CardContent>
...
export default function UserSettingsPage() {
  return (
    <div className=" mx-auto space-y-6">
      <div>
        <p className="text-base text-muted-foreground mt-0.5">Управление данными пользователя и компании</p>
      </div>
